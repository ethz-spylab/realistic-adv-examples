...
from RayS.dataset import load_binary_imagenet_test_data, BinaryImageNet
    ...
    for row_counter in range(patch_num):
        for col_counter in range(patch_num):
            noise_mask[row_counter][col_counter] = l2_distance(
                noise[:,
                (row_counter * patch_size):(row_counter * patch_size + patch_size),
        ...
        while step < iterations:
            ...
            if is_adversarial:
                ...
                noise_mask[best_row, best_col] = l2_distance(
                    best_noise[:,
                    (best_row * patch_size):(best_row * patch_size + patch_size),
                    (best_col * patch_size):(best_col * patch_size + patch_size)],
                    0)
                ...
                if binary_search:
                    ...
                    while torch.abs(torch.max(good_noise - bad_noise)) > 1:
                        ...
                        noises.append((bad_queries, current_min_noise.item()))
                        print("binary search", step, torch.abs(torch.max(good_noise - bad_noise)).item(),
