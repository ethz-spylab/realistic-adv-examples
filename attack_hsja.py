...
def construct_model_and_data(args):
    ...
    if args.model_name in KERAS_MODELS:
        model = KerasImageModel(args.model_name, args.dataset_name, train=False, load=True)
    elif args.model_name == "laion_nsfw":
        model = KerasV2ImageModel()

    # Split the test dataset into two parts.
    # Use the first part for setting target image for targeted attack.
    x_train, y_train, x_test, y_test = split_data(x_test,
                                                  y_test,
                                                  model,
                                                  num_classes=model.num_classes,
                                                  split_rate=0.5,
                                                  sample_per_class=np.min(
                                                      [np.max([200, args.num_samples // 10 * 3]), 1000]))
    ...
    if args.attack_type == 'targeted':
        ...
        target_images = [
            x_train_by_class[target_labels[j]][target_img_id] for j, target_img_id in enumerate(target_img_ids)
        ]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--stepsize_search',
                        type=str,
